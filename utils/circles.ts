import { Block, BlockPermutation, BlockType, BlockVolume, Dimension, Player, Vector3, world } from "@minecraft/server";
import { playerEcrDataManager } from "./playerDatabase";
import { processingManager } from "./processings";

export function createEmptyCircle(center: Vector3, radius: number, player: Player) {
...
export function createFilledCircle(center: Vector3, radius: number, player: Player) {
    let clipboard_block = playerEcrDataManager.getValue('clipboard_block', player);
    if (!clipboard_block) {
        return false;
    }
...
let circleBlockWaitList = new Set<BlockLocation>;
export function createOverClockedEmptyCircle(center: Vector3, radius: number, player: Player) {
    let clipboard_block = playerEcrDataManager.getValue('clipboard_block', player);
    if (!clipboard_block) {
        return;
    }
    ...
    for (let i = 0; i < 360; i += angleIncrement) {
        let radian = 2 * Math.PI * (i / 360);
        let newBlockLocation: BlockLocation = {
            target: {
