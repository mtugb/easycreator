import { Block, BlockPermutation, BlockType, BlockTypes, BlockVolume, Dimension, Player, Vector3, world } from "@minecraft/server";
...
export let REPLACE_MODES = ['selectedBlock', 'ice', 'snow', 'stone', 'grass', 'dirt', 'stoneAndOre', 'chest'] as const;
export type ReplaceMode = typeof REPLACE_MODES[number];
// グローバル変数
const blockWaitList: BlockWaitItem[] = [];
const limit = 100000;
let nn = 0;

// メインのdecay関数
export function decay(player: Player, loc: Vector3) {
    nn++;
    if (nn >= limit) {
        nn = 0;
    ...
    const existingIndex = blockWaitList.findIndex(item =>
        item.location.x === blockLocation.x &&
...
// ブロックチェックリストの生成
const getAdjacentLocations = (loc: BlockLocation): BlockLocation[] => {
    return [
        { ...loc, x: loc.x + 1 },
        { ...loc, x: loc.x - 1 },
        { ...loc, y: loc.y + 1 },
...
// ターゲットブロックの取得
function getTargetBlock(mode: ReplaceMode | null, player: Player): BlockPermutation | BlockType | null {
    if (!mode) return null;
    ...
    switch (mode) {
        ...
        case 'stoneAndOre': {
            ...
            if (randInt <= 2) return BlockTypes.get('minecraft:stone') ?? null;
            if (randInt <= 4) return BlockTypes.get('minecraft:andesite') ?? null;
